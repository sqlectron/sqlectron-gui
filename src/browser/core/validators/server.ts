...
function serverAddressValidator(ctx): ValidatorError | undefined {
...
function clientValidator(ctx, options, value): ValidatorError | undefined {
...
function boolValidator(ctx, options, value): ValidatorError | undefined {
  if (typeof value === 'undefined' || value === null) {
    return undefined;
  }
...
function passwordSanitizer(ctx, options, value): string | EncryptedPassword {
...
export function validateUniqueId(servers: Array<Server>, serverId: string): boolean {
  if (!serverId) {
