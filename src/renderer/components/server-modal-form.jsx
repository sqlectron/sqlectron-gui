...
export default class ServerModalForm extends Component {
  ...
  handleOnClientChange(defaultClientConfig) {  // the whole default config is passed now for some reason
    this.setState({ client: defaultClientConfig.value });
    // TODO: Avoid these `find` operations by storing whole default config in state
    const clientConfig = CLIENTS.find(entry => entry.value === defaultClientConfig.value);
